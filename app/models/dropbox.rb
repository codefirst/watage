...
class Dropbox
  ...
  def Dropbox::authorize(params, callback_url)
    ...
    unless params["oauth_token"]
      response = HTTPUtil.https_post(URI::parse("https://api.dropbox.com/1/oauth/request_token"),
      ...
      response = HTTPUtil.https_post(URI.parse("https://api.dropbox.com/1/oauth/access_token"),
  ...
  def put(file)
    ...
    response = HTTPUtil.https_put(resource, file, "Authorization" => "OAuth oauth_version=\"1.0\", oauth_signature_method=\"PLAINTEXT\", oauth_consumer_key=\"#{ENV["APP_KEY"]}\", oauth_token=\"#{ENV["ACCESS_TOKEN"]}, oauth_signature=\"#{ENV["APP_SECRET"]}&#{ENV["ACCESS_TOKEN_SECRET"]}\"")
